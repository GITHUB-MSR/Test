class Todo {
  ...
  {}) {
    this.nodes = {};
    this.title = title;
    this.data = this.loadFromLocalStorage() || data;
  ...
  handleDrop(e) {
    ...
    const draggedIndex = this.data.findIndex(task => task.id === draggedId);
    const targetIndex = this.data.findIndex(task => task.id === targetId);

    const [draggedTask] = this.data.splice(draggedIndex, 1);
    this.data.splice(targetIndex, 0, draggedTask);

    this.listUI(this.data);
    this.onOrderChanged(draggedId, targetId);
    this.saveToLocalStorage();
  }
    ...
    document.startViewTransition(() => {
      ...
      data.forEach(task => {
        const item = this.elementCreator({
          ...
          events: {
            dragstart: { action: this.handleDragStart.bind(this), api: true },
            dragover: { action: this.handleDragOver.bind(this), api: true },
            drop: { action: this.handleDrop.bind(this), api: true } },
...

{
  ...
  setInterval(() => {
    TodoApp.title = new Date().toLocaleTimeString();
    TodoApp.nodes.title.innerHTML = TodoApp.title;
  }, 1000);
...
TodoApp.onOrderChanged = (draggedId, targetId) => console.log("Order changed, draggedId:", draggedId, "targetId:", targetId);
